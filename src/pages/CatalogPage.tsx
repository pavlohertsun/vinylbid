      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-6 flex flex-wrap gap-4 items-center">
          <input
            ...
            value={search} onChange={e => { setSearch(e.target.value); if (e.target.value.length > 2) analytics.search(e.target.value); }}
          ...
          <div className="flex gap-1">
            {([['all', 'Всі'], ['dutch', 'Голландський'], ['vickrey', 'Vickrey']] as const).map(([val, label]) => (
              <button
                key={val}
                onClick={() => { setTypeFilter(val as typeof typeFilter); analytics.filterCatalog('auction_type', val); }}
          ...
          <select
            value={genre} onChange={e => { setGenre(e.target.value); analytics.filterCatalog('genre', e.target.value); }}
